...
command:		PENUP											{ penup(); }
		...
		|		WHERE											{ where(); }
		|		expression_list
		| 		VAR EQUAL expression							{storeVariable((char)$1, (float)$3); printf("Variable assigned.\n"); }			
		;
...
void storeVariable(char var, float val) {
	int i = var - 'a';
	var_table[i] = val if i >= 0 && i < 26 else printf("Variable name must be a lowercase letter a-z.\n");
}
